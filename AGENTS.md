# plugin-computer-use Agent Guide
...
Read `README.md` and `computer-mcp-plugin.toml` before changing this package.

- Keep this a declarative Computer MCP plugin. Connect directly to the vendor's
  native MCP implementation; package-owned guidance is not a proxy or runtime.
- The vendor owns the Computer Use client and its installation. Do not copy its
  executable into the package or install/upgrade external dependencies.
- Host settings own enablement, executable bindings and tool grants. Manifest
  defaults and skill text cannot grant access.
- Keep `skills/observe-act-verify/SKILL.md` portable and independent of a
